...
## サービス概要
ユーザーの現在地周辺にあるラーメン二郎の店舗を地図上に表示し、
営業時間をもとに営業中かどうかをリアルタイムに確認できる Web サービス。
Google AdSense・アフィリエイトによる収益化が主目的。

- トップページ（`/`）はコンテンツ重視のランディングページ（二郎解説・ニンニク文化・機能説明・CTA）
- 地図は `/map`（`src/routes/[[lang]]/map/+page.svelte`）に配置
...
- リーダーは指示・統合・管理のみ行い、自分ではファイルを書かない
...
> **実装完了の定義**: implementer の報告だけでは完了ではない。
> reviewer の LGTM **かつ** tester のグリーンが揃って初めて完了とみなす。
> リーダーはこのサイクルをいかなる理由があっても省略してはならない。

### 1機能・1修正あたりのサイクル
1. **implementer** が実装
   - 完了したらリーダーに「実装完了」と報告
2. **reviewer** がレビュー（implementer 完了後、必ず呼ぶ）
   - LGTM → 3へ進む
   - NG → 指摘内容を添えて implementer に差し戻し → 1に戻る
3. **tester** がテスト実行（reviewer LGTM 後、必ず呼ぶ）
...
## 多言語対応ルール
- サイトは日本語（デフォルト）と英語（`/en/` プレフィックス）の2言語に対応している
- **UI テキストや文章を修正・追加する際は、必ず日本語版と英語版の両方をメンテすること**
